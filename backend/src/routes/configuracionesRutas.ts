...
router.post('/', async (req: Request, res: Response) => {
    try {
        const { estatus, ubicacion } = req.body;
        const nueva = await configuracionesServices.agregarConfiguracion({ estatus, ubicacion });
        res.send(nueva);
    } catch (err) {
...
router.put('/', async (req: Request, res: Response) => {
    try {
        const { id, estatus, ubicacion } = req.body;
        const modificada = await configuracionesServices.modificarConfiguracion({ id, estatus, ubicacion });
        res.send(modificada);
    } catch (e) {
